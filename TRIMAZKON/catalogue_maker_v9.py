...
import db_read_catalogue_v2 as read_database
...
class Tools:
    ...
    def find_index(cls,given_db,acc_given,param_given="type"):
        """
        returns the index of a given string in given db based on given parameter
        """
        index = next((i for i, acc in enumerate(given_db)
                    if acc[param_given] == acc_given),
                    None
                    )
        return index
    ...
    class loading_routine:
        ...
        def create_window(self):
            ...
            # self.original_image = PILImage.open(Tools.resource_path("images/loading_x.png")).resize((100, 100))
...
class ToplevelWindow:
    ...
    def new_controller_window(self,childroot,controller = None,edit = False,accessory_index =0,only_accessory=False):
        ...
        """
        def server_db_part_presence(part,entry_given=None,device=""):
            try:
        ...
        def import_notes(operation = ""):
            ...
            if operation == "controller":
                ...
                if current_controller != "":
                    ...
                    #     None
                    # )
                    cont_index = Tools.find_index(self.controller_database,current_controller)
                    if cont_index != None:
                ...
                if current_accessory != "":
                    ...
                    #     None
                    # )
                    acc_index = Tools.find_index(self.whole_accessory_database,current_accessory)
        ...
        def initial_prefill():
            ...
            try:
                ...
                if "duplicity_id" in controller["accessory_list"][accessory_index]:
                    ids_on_entry[hw_type_entry][0] = controller["accessory_list"][accessory_index]["duplicity_id"]
                    ids_on_entry[hw_type_entry][1] = controller["accessory_list"][accessory_index]["type"]
                # else:
                #     hw_type_entry.delete(0,300)
                notes_input3.delete("1.0",tk.END)
...
class Catalogue_gui:
    ...
    def call_read_database(self,switch_manufacturer=False,loading_instance = None):
        ...
        def fill_lists(list_type,list_notes,db_list,key_name):
            ...
            list_notes.clear()
            seen_ids = set()
            db_list = [item for item in db_list if not (item["id"] in seen_ids or seen_ids.add(item["id"]))] #filtr proti stejným id duplicitám:
            
            types = [d["type"] for d in db_list]
            counts = Counter(types)
            duplicates = [t for t, count in counts.items() if count > 1]

            print("Duplicitní typy:", duplicates)
            for dup in duplicates:
                for items in db_list:
                    if dup == items["type"]:
                        self.duplicity_list.append(items["type"])
