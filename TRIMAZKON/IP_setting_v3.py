...
class Disk_management_gui:
    ...
    def refresh_disk_statuses(self):
        ...
        mapped_disks = Disk_management_tools.list_mapped_disks(whole_format = True)
        non_persistant_disks = Disk_management_tools.list_non_persistent_disks()
        for y in range(0,len(self.disk_letter_frame_list)):
            ...
            for i in range(0,len(non_persistant_disks)):
                if non_persistant_disks[i][0:1] == str(self.disk_all_rows[y][1]):
                    drive_status = Disk_management_tools.check_network_drive_status(non_persistant_disks[i])
            ...
            for i in range(0,len(mapped_disks)):
                if mapped_disks[i][0:1] == str(self.disk_all_rows[y][1]):
                    drive_status = Disk_management_tools.check_network_drive_status(mapped_disks[i])
                    if drive_status == True:
                        ...
                        param_frame.configure(fg_color = "red")
        if len(mapped_disks) == 0 and len(non_persistant_disks) == 0:
    ...
    def manage_bin(self,flag="",parameters=[],wb=None):
        ...
        def save_project_disk():
            ...
            if wb == None:
                return False
            Disk_management_tools.save_excel_data_disk(self.excel_file_path,
    ...
    def delete_disk_option_menu(self):
        ...
        mapped_disks = Disk_management_tools.list_mapped_disks()
        non_persistent_disks = Disk_management_tools.list_non_persistent_disks()
    ...
    def delete_project_disk(self,button_trigger = False,wanted_project=None,flag=""):
        ...
        def check_multiple_projects(window):
            ...
            if len(self.selected_list_disk) > 1:
                for names in name_list:
                    print(names)
                    project_found = False
                    self.disk_all_rows, self.disk_project_list, self.default_disk_status_behav = Disk_management_tools.read_excel_data(self.excel_file_path)
        ...
        # x = self.root.winfo_rootx()
        # y = self.root.winfo_rooty()
        # def_window_width = 500
        # print("trudu",int(proceed_label.winfo_width()))
        # if int(proceed_label.cget("width")) > def_window_width:
        #     def_window_width = int(proceed_label.cget("width"))
        # child_root.geometry(f"{def_window_width}x{child_root.winfo_height()}+{x+80}+{y+150}")
        child_root.grab_set()
        child_root.focus()
        child_root.focus_force()
    ...
    def save_new_project_data_disk(self,child_root,only_edit = None):
        ...
        if errors ==0:
            Disk_management_tools.read_excel_data(self.excel_file_path)
            if only_edit == None:
                Disk_management_tools.save_excel_data_disk(self.excel_file_path,
                                                        len(self.disk_all_rows),
                                                        self.last_project_id,
    ...
    def add_new_project_disk(self,edit = None):
        ...
        child_root.update()
        child_root.update_idletasks()
        # je to na gridu... nepujde...
        # child_root.geometry(f"{child_root.winfo_width()}x{child_root.winfo_height()}+{x+50}+{y+100}")
        child_root.focus()
        child_root.focus_force()
        self.root.bind("<Button-1>",lambda e: child_root.destroy(),"+")
    ...
    def setting_window(self):
        ...
        main_frame5 =   customtkinter.CTkFrame(master=child_root,corner_radius=0,border_color="#303030",border_width=2)
    ...
    def create_widgets_disk(self,init=None,disk_instance = None,ip_instance = None):
        if disk_instance != None:
            # self.disk_instance = disk_instance
            self.disk_instance = copy.copy(disk_instance)

        if ip_instance != None:
            # self.ip_instance = ip_instance
        ...
        button_switch_all_ip =          customtkinter.CTkButton(master = menu_cards, width = 200,height=50,text = "IP - všechny",command =  lambda: self.ip_instance.create_widgets(fav_status=False,disk_instance = self.disk_instance),font=("Arial",25,"bold"),corner_radius=0,fg_color="black",hover_color="#212121")
        button_switch_favourite_ip =    customtkinter.CTkButton(master = menu_cards, width = 200,height=50,text = "IP - oblíbené",command =  lambda: self.ip_instance.create_widgets(fav_status=True,disk_instance = self.disk_instance),font=("Arial",25,"bold"),corner_radius=0,fg_color="black",hover_color="#212121")
        ...
        project_label =         customtkinter.CTkLabel(master = self.main_widgets, width = 100,height=40,text = "Projekt: ",font=("Arial",20,"bold"))
        self.search_input =     customtkinter.CTkEntry(master = self.main_widgets,font=("Arial",20),width=160,height=40,placeholder_text="Název projektu",corner_radius=0)
        button_search =         customtkinter.CTkButton(master = self.main_widgets, width = 150,height=40,text = "Vyhledat",command =  lambda: self.make_project_first_disk("search"),font=("Arial",20,"bold"),corner_radius=0)
        button_add =            customtkinter.CTkButton(master = self.main_widgets, width = 150,height=40,text = "Nový projekt", command = lambda: self.add_new_project_disk(),font=("Arial",20,"bold"),corner_radius=0)
        button_remove =         customtkinter.CTkButton(master = self.main_widgets, width = 100,height=40,text = "Smazat", command =  lambda: self.delete_project_disk(button_trigger=True,flag="main_menu"),font=("Arial",20,"bold"),corner_radius=0)
        self.undo_button =      customtkinter.CTkButton(master = self.main_widgets, width = 50,height=40,text = "↶", command =  lambda: self.manage_bin(flag="load_deleted_disk"),font=("",28,"bold"),corner_radius=0,border_width=1,text_color="red")
        button_edit =           customtkinter.CTkButton(master = self.main_widgets, width = 110,height=40,text = "Editovat",command =  lambda: edit_project(),font=("Arial",20,"bold"),corner_radius=0)
        self.undo_edit =        customtkinter.CTkButton(master = self.main_widgets, width = 50,height=40,text = "↶", command =  lambda: self.manage_bin(flag="load_edited_disk"),font=("",28,"bold"),corner_radius=0,border_width=1,text_color="red")
        button_make_first =     customtkinter.CTkButton(master = self.main_widgets, width = 250,height=40,text = "Přesunout na začátek",command =  lambda: self.make_project_first_disk(),font=("Arial",20,"bold"),corner_radius=0)
        button_settings =       customtkinter.CTkButton(master = self.main_widgets, width = 40,height=40,text="⚙️",command =  lambda: self.setting_window(),font=("",22),corner_radius=0)
        delete_disk =           customtkinter.CTkButton(master = self.main_widgets, width = 250,height=40,text = "Odpojit síťový disk",command =  lambda: self.delete_disk_option_menu(),font=("Arial",20,"bold"),corner_radius=0,fg_color="red")
        reset =                 customtkinter.CTkButton(master = self.main_widgets, width = 200,height=40,text = "Reset exploreru",command = lambda: self.refresh_explorer(refresh_disk=True),font=("Arial",20,"bold"),corner_radius=0)
        self.refresh_btn =      customtkinter.CTkButton(master = self.main_widgets, width = 200,height=40,text = "Refresh statusů",command = lambda: self.refresh_disk_statuses(),font=("Arial",20,"bold"),corner_radius=0)
        as_admin_label =        customtkinter.CTkLabel(master = self.main_widgets,text = "",font=("Arial",20,"bold"))
        self.main_console =     tk.Text(self.main_widgets, wrap="none", height=0, width=180,background="black",font=("Arial",22),state=tk.DISABLED)
...
class IP_assignment: # Umožňuje měnit statickou IP
    ...
    def create_widgets(self,fav_status = None,init=None,excel_load_error = False,ip_instance = None,disk_instance = None):
        if ip_instance != None:
            self.ip_instance = copy.copy(ip_instance)
        if disk_instance != None:
            # self.disk_instance = disk_instance
        ...
        button_switch_disk =                customtkinter.CTkButton(master = menu_cards, width = 200,height=50,text = "Síťové disky",command =  lambda: self.disk_instance.create_widgets_disk(ip_instance = self.ip_instance),font=("Arial",25,"bold"),corner_radius=0,fg_color="black",hover_color="#212121")
        if excel_load_error:
        ...
        project_label =             customtkinter.CTkLabel(master = self.main_widgets, width = 100,height=40,text = "Projekt: ",font=("Arial",20,"bold"))
        self.search_input =         customtkinter.CTkEntry(master = self.main_widgets,font=("Arial",20),width=160,height=40,placeholder_text="Název projektu",corner_radius=0)
        button_search =             customtkinter.CTkButton(master = self.main_widgets, width = 150,height=40,text = "Vyhledat",command =  lambda: self.make_project_first("search"),font=("Arial",20,"bold"),corner_radius=0)
        self.button_add_main =      customtkinter.CTkButton(master = self.main_widgets, width = 150,height=40,text = "Nový projekt", command = lambda: self.add_new_project(),font=("Arial",20,"bold"),corner_radius=0)
        self.button_remove_main =   customtkinter.CTkButton(master = self.main_widgets, width = 100,height=40,text = "Smazat", command =  lambda: self.delete_project(button_trigger=True,flag="main_menu"),font=("Arial",20,"bold"),corner_radius=0)
        self.undo_button =          customtkinter.CTkButton(master = self.main_widgets, width = 50,height=40,text = "↶", command =  lambda: self.manage_bin(flag="load_deleted_ip"),font=("",28,"bold"),corner_radius=0,border_width=1,text_color="red")
        button_edit_main =          customtkinter.CTkButton(master = self.main_widgets, width = 110,height=40,text = "Editovat",command =  lambda: self.edit_project(),font=("Arial",20,"bold"),corner_radius=0)
        ...
        button_settings_behav =     customtkinter.CTkButton(master = self.main_widgets, width = 40,height=40,text="⚙️",command =  lambda: self.setting_window(),font=("",22),corner_radius=0)
        ...
        static_label =          customtkinter.CTkLabel(master = self.main_widgets, height=40,text = "Static:",font=("Arial",20,"bold"))
        self.static_label2 =    customtkinter.CTkLabel(master = self.main_widgets, height=40,text = "",font=("Arial",22,"bold"),bg_color="black")
        online_label =          customtkinter.CTkLabel(master = self.main_widgets, height=40,text = "Online: ",font=("Arial",22,"bold"))
        self.online_list =      customtkinter.CTkLabel(master = self.main_widgets, height=40,text = "",font=("Arial",22,"bold"))
        self.main_console =     tk.Text(self.main_widgets, wrap="none", height=0, width=180,background="black",font=("Arial",22),state=tk.DISABLED)
        main_menu_button.               pack(pady = (10,0),padx =(10,0),anchor = "s",side = "left")
...
class main:
    def __init__(self,root,menu_callback_function,window_mode,initial_path,zoom_factor):
        ...
        self.excel_file_path = initial_path + "config_TRIMAZKON.xlsx"
        ip_instance = IP_assignment(self.root,self.menu_callback,self.window_mode,self.initial_path)
        disk_instance = Disk_management_gui(self.root,self.menu_callback,self.window_mode,self.initial_path)
        default_environment = self.check_default_env()
        if default_environment == "disk":
            disk_instance.ip_instance = ip_instance
            disk_instance.create_widgets_disk(init=True,disk_instance=disk_instance)

        elif default_environment == "config_load_error":
