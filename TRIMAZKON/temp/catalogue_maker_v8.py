...
class Catalogue_gui:
    ...
    def read_database(self,switch_manufacturer=False):
        ...
        def fill_lists(list_type,list_notes,db_list,key):
            list_type.clear()
            list_notes.clear()
            seen_ids = set()
            ...
            for dup in duplicates:
                for items in db_list:
                    if dup == items["type"]:
                        self.duplicity_list[0].append(items["type"])
                        self.duplicity_list[1].append(items["id"])

            for items in db_list:
                list_type.append(items["type"])
                list_notes.append(items["description"])
                self.all_db_item_id_list[str(key)].append(items["id"])
        ...
        def save_changes(no_window_shut = False):
            ...
            if object == "camera" or all_parameters:
                ...
                if not camera_item in self.whole_camera_type_database and camera_item.replace(" ","") != "" and not server_db_part_presence(camera_item):
                    ...
                    self.temp_station_list[station_index]["camera_list"][camera_index]["type"] = camera_item
                    if camera_item in self.duplicity_list[0]:
                        self.temp_station_list[station_index]["camera_list"][camera_index]["id"] = self.all_db_item_id_list["camera_list"][self.whole_camera_type_database.index(camera_type)]
