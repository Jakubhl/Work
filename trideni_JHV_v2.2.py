# -verze 2.1 umožňuje v advanced módu třídění podle čísla kamery a podle cisla funkce
...
class sort_by_camera:
    ...
    def creating_folders(self):
        ...
        if "_Cam" in folder0_base:
            x = re.search("_Cam",folder0_base)
            delete_chars = x.span(0)[0]
        ...
        if "_Cam" in folder1_base:
            x = re.search("_Cam",folder1_base)
            delete_chars = x.span(0)[0]
       ...
       #     folder1_base = folder_name[1]     
        ...
        for j in range(0,len(self.Cam_number)):
            ...
            new_folder_name1 = folder1_base + "_Cam" + str(self.Cam_number[j])
            new_folder_name0 = folder0_base + "_Cam" + str(self.Cam_number[j])
...
class sort_by_function:
    ...
    def creating_folders(self):
        ...
        #pro Height
        if "_Cam" in folder_name[0]:
            x = re.search("_Cam",folder_name[0])
            delete_chars = x.span(0)[0]
            folder0_base = folder_name[0][0:delete_chars]   
        else:
            folder0_base = folder_name[0]
        ...
        #pro Normal
        if "_Cam" in folder_name[1]:
            x = re.search("_Cam",folder_name[1])
            delete_chars = x.span(0)[0]
            folder1_base = folder_name[1][0:delete_chars]
        else:
            folder1_base = folder_name[1]   
...
if advanced_mode.casefold() == "y":
    print("Třídit podle čísla funkce? (1) nebo podle čísla kamery? (2) nebo manuálně nastavit počet zakrytých znaků? (3):")
