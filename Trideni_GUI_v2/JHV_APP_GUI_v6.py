...
def menu():
    ...
    if data_read_in_txt[7] == "ano":
        ...
        root.after(0, lambda:root.self.state('zoomed')) # max zoom, porad v okne
...
class Image_browser:
    """
    Třída umožňuje procházet obrázky a přitom například vybrané přesouvat do jiné složky
    ...
    def call_browseDirectories(self):
    ...
    def calc_current_format(self,width,height):
    ...
    def view_image(self,increment_of_image):
        if len(self.all_images) != 0:
    ...
    def Reset_all(self):
        #global self.increment_of_image
        self.rotation_angle = 0.0
...
class Advanced_option:
    ...
    def setting_widgets(self,main_console_text,exception):
        ...
        path_set = customtkinter.CTkEntry(master = self.bottom_frame_default_path,width=700,height=30,placeholder_text="")
        ...
        if text_file_data[2] != False:
            ...
            path_set.configure(placeholder_text=str(text_file_data[2]))
            path_set.delete("0","200")
            path_set.insert("0", str(text_file_data[2]))
        else:
            default_path_insert_console.configure(text="Aktuálně nastavená základní cesta k souborům v Recources.txt je neplatná")
            path_set.configure(placeholder_text="Není nastavena žádná základní cesta")
        ...
        label_folder_cam = customtkinter.CTkLabel(master = self.bottom_frame_with_date,height=20,text = "Nastavte základní název složky pro třídění podle kamer:",justify = "left",font=("Arial",12,"bold"))
        set_new_def_prefix_cam = customtkinter.CTkEntry(master = self.bottom_frame_with_date,width=200,height=30, placeholder_text= str(default_prefix_cam))
        ...
        #console_cam_prefix=customtkinter.CTkLabel(master = self.bottom_frame_with_date,height=30,text ="",justify = "left",font=("Arial",12))
        ...
        label_folder_func = customtkinter.CTkLabel(master = self.bottom_frame_with_files_to_keep,height=20,text = "Nastavte základní název složky pro třídění podle funkce:",justify = "left",font=("Arial",12,"bold"))
        set_new_def_prefix_func = customtkinter.CTkEntry(master = self.bottom_frame_with_files_to_keep,width=200,height=30, placeholder_text= str(default_prefix_func))
        button_save_new_def_prefix_func = customtkinter.CTkButton(master = self.bottom_frame_with_files_to_keep,width=50,height=30, text = "Uložit", command = lambda: set_new_default_prefix("func"),font=("Arial",12,"bold"))
        #console_func_prefix=customtkinter.CTkLabel(master = self.bottom_frame_with_files_to_keep,height=30,text ="",justify = "left",font=("Arial",12))
        ...
        #console_pallets=customtkinter.CTkLabel(master = self.bottom_frame_sorting_formats,height=30,text ="",justify = "left",font=("Arial",12))
    ...
    def creating_advanced_option_widgets(self):
        #cisteni menu widgets
        ...
        label0          = customtkinter.CTkLabel(master = self.top_frame,height=20,text = "Nastavte požadované parametry (nastavení bude uloženo i po vypnutí aplikace): ",justify = "left",font=("Arial",16,"bold"))
        ...
        self.checkbox_maximalized.grid(column =0,row=1,sticky = tk.W,pady =0,padx=10)
...
class Converting_option:
    ...
    def create_convert_option_widgets(self):
        #cisteni menu widgets
        #for frames in self.list_of_menu_frames: 
        self.list_of_menu_frames[0].pack_forget()
        self.list_of_menu_frames[0].grid_forget()
...
class Deleting_option:
    ...
    def del_files(self,path):
        ...
        Deleting.whole_deleting_function(path,self.more_dirs,del_option,self.files_to_keep,self.cutoff_date,self.supported_formats_deleting,testing_mode)
        output_text = ""
    ...
    def selected(self,console_frame_right_1_text,console_frame_right_2_text):
    ...
    def selected2(self,console_frame_right_1_text,console_frame_right_2_text):
        self.clear_frame(self.frame_right)
        self.console.configure(text = " ")
    ...
    def selected3(self,console_frame_right_1_text,console_frame_right_2_text):
        self.clear_frame(self.frame_right)
        self.console.configure(text = " ")
        self.checkbox2.deselect()
        self.checkbox.deselect()
    ...
    def selected6(self):
        if self.checkbox6.get() == 1:
            if self.checkbox3.get() == 1:
                self.info2.configure(text = "- Pro tuto možnost třídění není tato funkce podporována",font=("Arial",16,"bold"))
            else:
                self.info2.configure(text = "- VAROVÁNÍ: Máte spuštěné možnosti mazání souborů i ve všech subsložkách vložené cesty (max:6 subsložek)",font=("Arial",16,"bold"))
        else:
            self.info2.configure(text = "")

    def create_deleting_option_widgets(self):
        #cisteni menu widgets
        #for frames in self.list_of_menu_frames: 
        self.list_of_menu_frames[0].pack_forget()
        self.list_of_menu_frames[0].grid_forget()
...
class Sorting_option:
    ...
    def sort_files(self,path):
        ...
        Trideni.whole_sorting_function(path,selected_sort,self.more_dirs,self.max_num_of_pallets,self.by_which_ID_num,self.prefix_func,self.prefix_Cam,self.supported_formats_sorting,self.aut_detect_num_of_pallets)
        output_text = ""
        output_text2 = ""
    ...
    def selected(self): #tridit podle typu souboru
    ...
    def selected2(self): #tridit polde cisla funkce (ID)
        self.clear_frame(self.frame6)
        self.console.configure(text = " ")
        self.view_image(2)
        self.checkbox.deselect()
    ...
    def selected3(self): #tridit podle cisla kamery
        self.clear_frame(self.frame6)
        self.console.configure(text = " ")
        self.view_image(3)   
        self.checkbox.deselect()
    ...
    def selected4(self): #tridit podle obojiho
        self.clear_frame(self.frame6)
        self.console.configure(text = " ")
        self.view_image(4)
        self.checkbox.deselect()
    ...
    def selected5(self): #hledani paru
        ...
        button_save1        = customtkinter.CTkButton(master = self.frame6,width=50,height=30, text = "Uložit", command = lambda: set_pair_variable1(),font=("Arial",12,"bold"))
    ...
    def create_sorting_option_widgets(self):
        # cisteni menu widgets:
        self.list_of_menu_frames[0].pack_forget()
        self.list_of_menu_frames[0].grid_forget()
        self.list_of_menu_frames[0].destroy()
