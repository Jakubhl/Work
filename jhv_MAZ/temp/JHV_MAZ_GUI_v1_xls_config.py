...
class Tools:
    ...
    @classmethod
    def create_new_config(cls,default_value_list,default_labels):
        ...
        wb = Workbook()
        ws = wb.create_sheet(title=cls.setting_list_name)
        ...
        wb.save(initial_path+cls.config_filename)
        wb.close()

    @classmethod
    def read_config_data(cls): # Funkce vraci data z config.
        """
        Funkce vrací data z konfiguračního souboru config_MAZ.xlsx
        ...
        if os.path.exists(initial_path+cls.config_filename):
            try:
                cutoff_date = ["","",""]
                supported_formats_deleting = []
                wb = load_workbook(initial_path+cls.config_filename)
                ws = wb[cls.setting_list_name]
                ...
                print(f"Nejdřív zavřete soubor {cls.config_filename} Chyba: {e}")   
                print("Budou načteny defaultní hodnoty")
                global_recources_load_error = True
                output_array = load_default_values()
                return output_array
        else:
            print(f"Chybí konfigurační soubor {cls.config_filename}")
            print("Bude vytvořen")
            # print("Budou načteny defaultní hodnoty")
            # global_recources_load_error = True
            Tools.create_new_config(default_setting_parameters,default_labels)
            output_array = load_default_values()
            return output_array
        ...
        def save_task_to_config():
            ...
            try:
                ...
                if success_status:
                    ...
                    wb.save(initial_path + self.config_filename)
                else:
