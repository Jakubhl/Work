...
class Tools:
    ...
    def remove_task_from_TS(cls,name_of_task):
        ...
        ps_command = f"Unregister-ScheduledTask -TaskName \"{name_of_task}\" -Confirm:$false"
        ...
        if Tools.is_admin(): # pokud se vypíná admin app - vypnout i admin tray a zapnout bez práv
            ...
            if data_read_in_config[9] == "ano":
                ...
                try:
                    ...
                    print("Running task with command:", ps_command)
    ...
    def save_new_task(self):
        ...
        def save_task_to_config():
            ...
            def set_up_task_in_ts():
                ...
                exe_path = f'"{path_app_location}"'
                arguments = (
                    f"deleting {task_name} {new_task['operating_path']} {new_task['max_days']} "
                    f"{new_task['files_to_keep']} {new_task['more_dirs']} {new_task['selected_option']} {new_task['creation_date']}"
                )

                # Construct the PowerShell command.
                # - New-ScheduledTaskAction takes the executable (with -Execute) and its arguments (with -Argument).
                # - New-ScheduledTaskTrigger sets up a daily trigger at the specified time.
                ps_command = (
                    f"$action = New-ScheduledTaskAction -Execute {exe_path} -Argument '{arguments}'; "
                    f"$trigger = New-ScheduledTaskTrigger -Daily -At '{repaired_freq_param}'; "
